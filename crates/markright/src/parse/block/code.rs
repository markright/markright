use crate::ast::block::Block;
use crate::parse::Parser;

const DIAGRAM_LANGS: &[&str] = &["mermaid", "dot", "graphviz", "plantuml"];

impl<'a> Parser<'a> {
    pub(crate) fn try_code_block(&mut self) -> Option<Block<'a>> {
        let line = self.current_line();
        let fence_len = line.bytes().take_while(|&b| b == b'`').count();
        if fence_len < 3 {
            return None;
        }

        let info = line[fence_len..].trim();
        let lang = if info.is_empty() { None } else { Some(info) };

        self.advance_line();
        let body = self.consume_fenced_body(|candidate| {
            let close_len = candidate.bytes().take_while(|&b| b == b'`').count();
            close_len >= fence_len && candidate.len() == close_len
        });

        if let Some(lang_str) = lang
            && DIAGRAM_LANGS.contains(&lang_str)
        {
            return Some(Block::DiagramBlock {
                lang: lang_str,
                body,
                id: None,
            });
        }

        Some(Block::CodeBlock {
            lang,
            body,
            id: None,
        })
    }
}
